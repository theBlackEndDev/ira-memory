...
```
Assistant (IRA / OpenClaw)
        |
   Memory API  ── store() / recall() / search() / summarize()
        |
  PostgreSQL 16 + pgvector + pg_trgm
   ├── sessions          append-only session log
   ├── messages           append-only message log
   ├── memory_facts       curated knowledge (decisions, preferences, plans, todos)
   ├── summaries          compacted representations (session, daily, weekly, project)
   ├── *_embeddings       vector tables for semantic search (1536-dim, text-embedding-3-small)
   └── file_sync_records  tracks file-based memory imports
```
...
The database runs as `ira-memory-db` in the orchestrator's `infrastructure/docker-compose.yml` on port **5433** (not 5432, to avoid conflicts with other Postgres instances).
...
│   ├── summarize.ts           LLM summarization (session, daily, weekly, project)
